...
function backend_variant(JuMPModel)
    ...
    print("Solving With : JuMPDefault ($(MOI.get(JuMPModel , MOI.AutomaticDifferentiationBackend())))...")
    ...
    print("Solving With : SymbolicAD ($(MOI.get(JuMPModel , MOI.AutomaticDifferentiationBackend())))...")
    b = @benchmark optimize!(JuMPModel)
    tt = prettytime(median(b.times));
	tf = median(b.times);
    diff_auto_time = solve_time(JuMPModel);
    ...
    """"""
    sort!(be_results, [:tf])
    printstyled("-------------------------Results of Backends-------------------------",color = :blue)
	println()
