...
function solving_model_OC(model;max_iter=1000, tol=1e-8, constr_viol_tol = 1e-6,solver="ma57",display=false)
    # Solve the problem
    printlevel = 0
    if display
        printlevel = 5
...
function benchmark_model_OC(model_function, nb_discr_list;max_iter=1000, tol=1e-8, constr_viol_tol = 1e-6,solver="ma57",display=false)
    DataModel = []
    ...
    for nb_discr in nb_discr_list
        ...
        # Solve the model
        nb_iter, total_time, Ipopt_time, obj_value, flag, nvar, ncon = solving_model_OC(model;max_iter=max_iter, tol=tol, constr_viol_tol = constr_viol_tol,solver=solver,display=display)
        # Save the data
...
function benchmark_all_models_OC(models, nb_discr_list, excluded_models;max_iter=1000, tol=1e-8, constr_viol_tol = 1e-6,solver="ma57",display=false)
    ...
    for (k,v) in models
        ...
        info = benchmark_model_OC(v, nb_discr_list;max_iter=max_iter, tol=tol, constr_viol_tol = constr_viol_tol,solver=solver,display=display)
        Results[k] = info
        println("✅")
    end
    return Results
