...
function generate_template_problem(
    ...
    if isdir(figures_dir)
        ...
        for filename in files
            ...
            ```@raw html
            <a href="$assets_href_prefix/benchmarks/$bench_id/figures/$base.pdf">
              <img 
                class="centering" 
                width="100%" 
                style="max-width:1400px" 
                src="$assets_href_prefix/benchmarks/$bench_id/figures/$base.svg"
              />
            </a>
            ```
            """
...
"""
    generate_template_problem_from_list(problem_name, benchmarks, title, desc, src_dir, assets_href_prefix; draft)
...
function write_core_benchmark_templates(
    ...
    # Compute relative href prefix from problem pages to assets directory
    assets_dir = joinpath(src_dir, "assets")
    assets_href_prefix = replace(relpath(assets_dir, problems_dir), "\\" => "/")
