...
function _plot_performance_profiles(bench_id)
    ...
    function performance_profile(df)
        ...
        function _compute_ratio_limits()
            min_ratio = Inf
            max_ratio = 1.0
            for c in combos
        ...
        function _marker_indices_for_curve(ratios, min_ratio, max_ratio, curve_idx, n_curves; M = 6)
            n = length(ratios)
            if n == 0
                return Int[]
            elseif n == 1
        ...
        min_ratio, max_ratio = _compute_ratio_limits()
        ...
        for (idx, c) in enumerate(combos)
            ...
            if !isempty(ratios)
                ...
                marker_indices = _marker_indices_for_curve(ratios, min_ratio, max_ratio, idx, n_curves)
