...
"""
    replace_analysis_blocks(content::String) -> String

Replace all INCLUDE_ANALYSIS blocks in the content with Markdown generated by
...
```html
<!-- INCLUDE_ANALYSIS:
FUNCTION = "_analyze_performance_profiles"
...
analysis function.
"""
function replace_analysis_blocks(content::String)
    pattern = r"<!-- INCLUDE_ANALYSIS:\s*\n(.*?)-->"s
    block_count = 0

    result = replace(content, pattern => function(match_str)
        block_count += 1
        @info "🧮 Processing INCLUDE_ANALYSIS block #$block_count"

        m = match(pattern, match_str)
        if m === nothing
            @warn "  ✗ Failed to parse INCLUDE_ANALYSIS block"
            return match_str
        end
        ...
        if function_name === nothing
            @warn "  ✗ Missing FUNCTION parameter in INCLUDE_ANALYSIS block"
            return match_str
        end
        ...
        try
            analysis_md = call_analysis_function(function_name, args)
            @info "  ✓ Replaced INCLUDE_ANALYSIS block #$block_count with analysis markdown"
            return analysis_md
        catch e
    ...
    @info "🧮 Replaced $block_count INCLUDE_ANALYSIS block(s)"
    return result
end
...
function process_single_template(
    ...
    # Replace all INCLUDE_ANALYSIS blocks
    processed_content = replace_analysis_blocks(processed_content)
