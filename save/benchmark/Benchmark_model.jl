...
function benchmark_1model_JuMP(
    model,
    nb_discr;
    max_iter=1000,
    tol=1e-8,
    ...
    # Solve the model
    t = @timed(alloc = @allocated(optimize!(model)))
    # Get the results
...
function benchmark_model(
    ...
    for model_key in model_key_list
        ...
        for nb_discr in nb_discr_list
            ...
            info_JuMP, a = benchmark_1model_JuMP(
                model,
                nb_discr;
                max_iter=max_iter,
                tol=tol,
...
# Function to benchmark the model
function benchmark_model_TTonly(
    model_key_list,
    nb_discr_list;
    max_iter=1000,
    tol=1e-8,
    ...
    for model_key in model_key_list
        ...
        for nb_discr in nb_discr_list
            ...
            info_OC, allocOC = benchmark_1model_OC(
                model,
                nb_discr;
                max_iter=max_iter,
                tol=tol,
            ...
            print("Benchmarking the model $model_key with JuMP ($nb_discr)... ")
            model = functions_list[model_key](JuMPBackend(); nh=nb_discr)
            info_JuMP, allocJuMP = benchmark_1model_JuMP(
                model,
                nb_discr;
                max_iter=max_iter,
                tol=tol,
            ...
            push!(
                ...
                DataFrame(
                    :nb_discr => nb_discr,
                    :TTJMP => info_JuMP.total_time,
                    :TTOC => info_OC.total_time,
                    :IterJuMP => info_JuMP.nb_iter,
                    :IterOC => info_OC.nb_iter,
                    :AllocJuMP => allocJuMP,
                    :AllocOC => allocOC,
                ),
            )
