...
function display_Benchmark(Results, title, file_name, parameter_value)
            ...
            for i in v
                push!(
                    ...
                    [
                        ...
                        i.ncon;
                        i.nb_iter[1];
                        round(i.total_time[1]; digits=2);
                        round(i.Ipopt_time[1]; digits=2);
                        i.obj_value[1];
                        uniflag(i.flag[1])
                    ],
                )
    ...
    hl_flags = LatexHighlighter(
        (table, i, j) ->
            ((j == 9) && (table[i, j] != "Solve Succeeded") && (table[i, j] != "NaN")),
        ["color{red}"],
    );
...
function display_Benchmark_model(Results, title, file_name, parameter_value)
    ...
    hl_flags = LatexHighlighter(
        (table, i, j) ->
            ((j == 10) && (table[i, j] != "Solve Succeeded") && (table[i, j] != "NaN")),
        ["color{red}"],
    );
    original_stdout = stdout
    file = open("./outputs/$(file_name)", "w")
        ...
        for i in s
            push!(
                ...
                [
                    k;
                    i.nb_discr;
                    round(i.TTJMP[1]; digits=2);
                    round(i.TTOC[1]; digits=2);
                    i.IterJuMP[1];
                    i.IterOC[1];
                    round(i.AllocJuMP[1]/1e6; digits=1);
                    round(i.AllocOC[1]/1e6; digits=1)
                ],
            )
        end
    end
        ...
        for i in v
            push!(
                ...
                [
                    ...
                    i.nnzh;
                    i.nnzj;
                    round(i.t_obj[1]*1e3; digits=2);
                    round(i.t_grad[1]*1e3; digits=2);
                    round(i.t_cons[1]*1e3; digits=2);
                    round(i.t_jac[1]*1e3; digits=2);
                    round(i.t_hess[1]*1e3; digits=2)
                ],
            )
        end
    end
        ...
        for i in v
            push!(
                ...
                [
                    ...
                    i.nb_iter[1];
                    round(i.total_time[1]; digits=2);
                    round(i.knitro_time[1]; digits=2);
                    i.obj_value[1];
                    uniflag(i.flag[1])
                ],
            )
