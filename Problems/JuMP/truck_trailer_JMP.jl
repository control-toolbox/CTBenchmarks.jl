...
# parameters
    ...
    @variables(model, begin 
        ...
        x2[0:nh]              
        y2[0:nh]   
        -pi/2 <= theta0[0:nh] <= pi/2
        -pi/2 <= theta1[0:nh] <= pi/2
        theta2[0:nh]
        -0.2 * speedf <= v0[0:nh] <= 0.2 * speedf
    ...
    @expression(model,v1[j=0:nh], v0[j] * cos(delta0[j]) + M0 * sin(delta0[j]) * dtheta0[j])
    ...
    @objective(model, Min, tf + sum(beta01[j]^2 for j in 0:nh) + sum(beta12[j]^2 for j in 0:nh))
