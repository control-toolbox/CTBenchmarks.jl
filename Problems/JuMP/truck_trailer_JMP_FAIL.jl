...
# parameters
    ...
    @constraints(model,begin
        ...
        d_v0[j=1:nh],       v0[j] == v0[j-1] + 0.5 * step * (v0_dot[j] + v0_dot[j-1])
        d_delta0[j=1:nh],   delta0[j] == delta0[j-1] + 0.5 * step * (delta0_dot[j] + delta0_dot[j-1])
    end)
